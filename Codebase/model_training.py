...
eng_train,eng_test,y_train,y_test=train_test_split(eng_padded_seq,y,test_size=0.05)

#defining architecture of network
model=Sequential()
model.add(Embedding(input_dim=len(eng2index),output_dim=22, input_length= max_len))
model.add(Dropout(0.20))
#model.add(Conv1D(64,4,activation="relu",padding="same"))
model.add(Bidirectional(LSTM(units=128,return_sequences=True,recurrent_dropout=0.25)))
model.add(TimeDistributed(Dense(len(hindi2index),activation="softmax")))
model.summary()

model.compile(optimizer="rmsprop",loss="categorical_crossentropy",metrics=["accuracy"])
history=model.fit(eng_train,np.array(y_train),validation_data=(eng_test,np.array(y_test)),batch_size=32,epochs=20,verbose=1)
